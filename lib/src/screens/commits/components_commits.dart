...
class _RepositoryFilterBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      children: [
        _RepositoryRow(onTap: onTap, repository: null, selectedRepository: selectedRepository),
        ...
        ...groupedRepos.entries.map((entry) {
          ...
          return Column(
            ...
            children: [
              Text(projectName, style: context.textTheme.titleMedium),
              ...
              ...repos.map(
                (repo) => Padding(
                  ...
                  child: Column(
                    children: [
                      _RepositoryRow(repository: repo, selectedRepository: selectedRepository, onTap: onTap),
                      if (repo != repos.last) const Divider(height: 24) else const SizedBox(height: 24),
                    ],
                  ),
                ),
              ),
...
class _RepositoryRow extends StatelessWidget {
  const _RepositoryRow({required this.repository, required this.selectedRepository, required this.onTap});
    ...
    return InkWell(
      ...
      child: Row(
        children: [
          ...
          if (isSelected) ...[const Spacer(), Icon(DevOpsIcons.success)],
        ],
      ),
    );
  }
